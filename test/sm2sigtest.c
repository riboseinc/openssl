...
uint8_t* fake_rand_bytes = NULL;
size_t fake_rand_bytes_offset = 0;
...
static EC_GROUP *create_EC_group(
   const char *p_hex, const char *a_hex, const char *b_hex,
   const char *x_hex, const char *y_hex,
   const char *order_hex, const char *cof_hex)
   {
   BIGNUM* p = NULL;
   BN_hex2bn(&p, p_hex);
   BIGNUM* a = NULL;
...
int test_sm2(const EC_GROUP* group,
   ...
   ECDSA_SIG* sig = SM2_do_sign(key, EVP_sm3(), userid, (const uint8_t*)message, strlen(message));
   restore_rand();
   ...
   int ok = SM2_do_verify(key, EVP_sm3(), sig, userid, (const uint8_t*)message, strlen(message));
